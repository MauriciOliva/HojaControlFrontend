...
const FormatoControlVisita = () => {
  ...
  const generarExcelLocalmente = async () => {
    try {
      ...
      // FUNCIÓN PARA INSERTAR IMÁGENES DE FIRMAS
      const insertarImagenEnCelda = async (worksheet, imageDataUrl, cellAddress) => {
        ...
        try {
          ...
          // Insertar imagen en la celda específica
          // Ajusta estas coordenadas según donde deben ir las firmas en tu plantilla
          // Puedes ajustar el tamaño cambiando estos valores:
          worksheet.addImage(imageId, {
            tl: { col: worksheet.getCell(cellAddress).col - 1, row: worksheet.getCell(cellAddress).row - 1 },
            br: {
              col: worksheet.getCell(cellAddress).col + 5, // ← Más ancho (aumenta este número)
              row: worksheet.getCell(cellAddress).row + 2  // ← Más alto (aumenta este número)
            },
            editAs: 'oneCell'
          });
        } catch (error) {
      ...
      // INSERTAR LAS FIRMAS - AJUSTA ESTAS CELDAS SEGÚN TU PLANTILLA
      await insertarImagenEnCelda(worksheet, firmaCliente, 'B43');     // Firma cliente
      await insertarImagenEnCelda(worksheet, firmaTecnico, 'I43');     // Firma técnico
      await insertarImagenEnCelda(worksheet, firmaAlmacenista, 'AB43'); // Firma almacenista
  ...
  return (
    <div className="container mx-auto p-4 bg-white shadow-lg rounded-md max-w-5xl">
      {/* Mensaje de estado */}
      {mensaje.texto && (
        <div className={`mb-4 p-3 rounded-md ${mensaje.tipo === 'error' ? 'bg-red-100 text-red-700' :
